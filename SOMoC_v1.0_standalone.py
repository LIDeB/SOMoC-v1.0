...
input_file = "test/focal_adhesion.csv"
...
def Get_input_data():
    ...
    if input_file is not None:
        ...
        name = Get_name("test/focal_adhesion.csv")
        data = pd.read_csv("test/focal_adhesion.csv",
...
def Standardize_molecules(data):
    ...
    print('By default SOMoC will standardize molecules before fingerprint calculation. However, you can disable standardization by setting clean=Flase.')
...
def Fingerprints_calculator(data):
    ...
    try:
        ...
        print("**Oh no! There was a problem with Fingerprint calculation of some smiles.**  :confused:")
        print(" :point_down: **Try using our standarization tool before clustering **")
...
def GMM_clustering_final(embeddings, K):
    ...
    if GMM_final.converged_:
        ...
        print(f'GMM did not converge. Please check you input configuration.')
    ...
    data_clustered.to_csv(f'results/{name}_Clustered_SOMoC.csv', index=True, header=True)
    table_metrics.to_csv(f'results/{name}_Validation_SOMoC.csv', index=True, header=True)
...
def Elbow_plot(results):
    ...
    fig.write_html(f'results/{name}_elbowplot_SOMoC.html')
...
def Distribution_plot(data_clustered):
    ...
    fig.write_html(f'results/{name}_size_distribution_SOMoC.html')

    sizes.to_csv(f'results/{name}_Size_distribution_SOMoC.csv', index=True, header=True)  # Write the .CSV file
...
def Setting_info():
    ...
    settings_df.to_csv(f'results/{name}_Settings_SOMoC.csv', index=True, header=False)
